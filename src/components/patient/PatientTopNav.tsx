...
import { Globe, Menu, ArrowLeft, Home, User, Settings, LogOut } from 'lucide-react';
import { useLanguage } from '../../contexts/LanguageContext';
import NotificationBell from '../notifications/NotificationBell';
import { PARNIA_NOTIFICATIONS, PatientNotification } from '../../data/patientNotifications';
...
export default function PatientTopNav({ patientName = 'Ahmed Al Maktoum' }: PatientTopNavProps) {
  ...
  useEffect(() => {
    const close = () => { setShowPortalMenu(false); setShowAvatar(false); };
      ...
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <div className="relative">
            <button
              onClick={() => { setShowPortalMenu(!showPortalMenu); setShowAvatar(false); }}
          ...
          <div className={`${isRTL ? 'pr-4 border-r' : 'pl-4 border-l'} border-gray-200 relative`}>
            <button
              onClick={() => { setShowAvatar(!showAvatar); setShowPortalMenu(false); }}
