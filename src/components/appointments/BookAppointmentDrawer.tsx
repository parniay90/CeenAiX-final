...
import { SPECIALTIES, MOCK_DOCTORS, MOCK_TIME_SLOTS, Doctor, AppointmentDetail } from '../../types/appointments';

interface BookAppointmentDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onBooked: (appointment: AppointmentDetail) => void;
}

export default function BookAppointmentDrawer({ isOpen, onClose, onBooked }: BookAppointmentDrawerProps) {
  ...
  const handleConfirmBooking = () => {
    ...
    const newAppointment: AppointmentDetail = {
      ...
      clinicAddress: `${selectedDoctor.hospital}, Dubai`,
      date: selectedDate,
      time: selectedTime,
      type: isTeleconsult ? 'Teleconsult' : 'In-Person',
      status: 'Awaiting Confirmation',
      prepNotes: reasonForVisit ? `Reason for visit: ${reasonForVisit}` : undefined,
    };
    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-2xl bg-white shadow-xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {SPECIALTIES.map((specialty) => (
                  <button
                    key={specialty.id}
                    onClick={() => { setSelectedSpecialty(specialty.name); setStep(2); }}
          ...
          {step === 2 && (
            <div>
              <button onClick={() => setStep(1)} className="mb-4 text-teal-600 hover:text-teal-700 font-medium">
                ← Back to Specialties
              </button>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          ...
                          {doctor.inNetwork && (
                            <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded">In-Network</span>
                          )}
                        </div>
          ...
          {step === 3 && selectedDoctor && (
            <div>
              <button onClick={() => setStep(2)} className="mb-4 text-teal-600 hover:text-teal-700 font-medium">
                ← Back to Providers
              </button>
              ...
              <div className="mb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-medium text-gray-700">Schedule as Teleconsult (Video Call)</span>
                </label>
              </div>
              <div className="mb-4">
          ...
          {step === 4 && (
            <div>
              <button onClick={() => setStep(3)} className="mb-4 text-teal-600 hover:text-teal-700 font-medium">
                ← Back to Date & Time
              </button>
              <h3 className="text-lg font-bold text-gray-900 mb-4">Reason for Visit</h3>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">Describe your symptoms or reason for visit</label>
                <textarea
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Fever', 'Cough', 'Headache', 'Fatigue', 'Pain', 'Other'].map((symptom) => (
                    <button
                      key={symptom}
                      onClick={() => setReasonForVisit(prev => prev ? `${prev}, ${symptom}` : symptom)}
              ...
              <div className="bg-gray-50 rounded-lg p-6 mb-6">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-900">{format(selectedDate, 'MMMM dd, yyyy')}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-900">{isTeleconsult ? 'Teleconsult' : 'In-Person'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {reasonForVisit && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-gray-900 text-right max-w-xs">{reasonForVisit}</span>
                    </div>
                  )}
                </div>
              </div>
